...
class JFormFieldQuestionSort extends JFormField
{
	protected $type = 'QuestionSort';
	...
	protected function getInput()
	{
		...
		$query->where('q_type IN ("textbox","email","cbox","datedropdown")');
		$query->order('ordering');
		$db->setQuery($query);
		$html[] = '<select name="'.$this->name.'" class="form-select" '.$attr.'>';
        $html[] = JHtml::_('select.options',[['value'=>"0",'text'=>"Submission Time"],['value'=>"-1",'text'=>"Featured"]],"value","text",$this->value);
